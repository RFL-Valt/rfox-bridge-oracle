...
/*
This oracle listens to the ethereum blockchain for `bridge` events.
...
// const bridge_topic =
//   "0x622824274e0937ee319b036740cd0887131781bc2032b47eac3e88a1be17f5d5"; 0xd8a9e8b54ce268761ce8779a976539ca5ffd8eff182757353023f1044959267d

const bridge_topic =
  "0xd8a9e8b54ce268761ce8779a976539ca5ffd8eff182757353023f1044959267d";
  ...
  return new Promise(async (resolve, reject) => {
    try {
      const query = {
        ...
        toBlock: to_block,
        address: config.eth.bridgeContract,
...
const process_bridgeed = async (from_block, to_block) => {
  return new Promise(async (resolve, reject) => {
    try {
      const query = {
        fromBlock: from_block,
        ...
        for (let r = 0; r < res.length; r++) {
          ...
          await_confirmation(res[r].transactionHash).then(async () => {
            try {
              ...
              console.log(
                `Sent notification of bridge with txid ${eos_res.transaction_id}`
              );
              // resolve();
            } catch (e) {
              if (e.message.indexOf("Oracle has already approved") > -1) {
                ...
                console.error(`Error sending bridge ${e.message}`);
                // reject(e);
              }
            }
          });
...
const run = async (from_block = "latest") => {
  while (true) {
    try {
      ...
      await process_bridgeed(from_block, to_block);
