...
/*
Lists all incomplete bridges from eos -> eth
 */
...
const run = async () => {
  ...
  for (let i = 0; i < incomplete.length; i++) {
    try {
      const url = `${hyperion_endpoint}/v2/history/get_actions?account=${incomplete[i].account}&filter=other.worlds:bridge&count=100`;
      const res = await fetch(url);
      const json = await res.json();
      // console.log(json.actions[0])
      const missed = json.actions
        .map((a) => {
          return { act: a.act, block_num: a.block_num };
        })
        .filter(
          (t) =>
            t.act.data.quantity === incomplete[i].quantity &&
            t.act.data.eth_address.toLowerCase() ===
