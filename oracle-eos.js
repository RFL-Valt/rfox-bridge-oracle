...
/*
This oracle listens to the EOSIO chain for bridge actions using a state history node

After receiving a bridge action, it will sign the data and then send it to the EOS chain where the client can pick
...
class TraceHandler {
  ...
  async processQueue() {
    ...
    try {
      ...
      const chain_data = bridge_res.rows[0];
  ...
  async sendSignature(data, data_serialized, retries = 0) {
    console.log(
      data,
      data_serialized,
      Buffer.from(data_serialized).toString("hex")
    );
            ...
            switch (action[0]) {
              case "action_trace_v0":
                if (
                  ...
                  console.log(
                    `Sending ${action_deser[0].data.quantity} to ${action_deser[0].data.eth_address}`,
                    action_deser[0].data
                  );
...
const run = async (config) => {
  console.log(
    `Starting EOS watcher for ETH oracle ${config.eth.oracleAccount}`
  );
